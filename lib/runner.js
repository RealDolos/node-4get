...
class Timers {
  ...
  new(timeout) {
    ...
    timer.finally(() => this.timers.delete(timer)).ignore();
    return timer;
  }
  cancel() {
...
class Runner {
  constructor(options) {
    ...
    this.pools = new Pools(this.convert);
    Object.seal(this);
  ...
  getMedia(post) {
    return this.pools.media.schedule(this._getMedia.bind(this, post));
  }
  ...
  async monitorThreads(threads) {
    ...
    while (!this.cancelled) {
      for (const thread of threads) {
        ...
        waiting.add(pp);
        pp.catch(log.error).finally(() => waiting.delete(pp)).ignore();
      }
      if (!this.cancelled) {
